...
export const categoryService = {
  ...
  async create(data: { name: string; slug: string; description?: string; parentId?: number | null; displayOrder?: number; isActive?: boolean }) {
  ...
  async update(id: number, data: { name?: string; slug?: string; description?: string; parentId?: number | null; displayOrder?: number; isActive?: boolean }) {
    const category = await categoryRepository.findById(id);
    if (!category) throw new AppError("Category not found", 404);
